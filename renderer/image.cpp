...
static unsigned char read_byte(std::string str, unsigned long index) {
    return (unsigned char)str.at(index);
}
...
static void load_tga_rle(std::string str, image_t *image) {
    ...
    int buffer_count = 0;
    int i, j;
    unsigned long index = TGA_HEADER_SIZE;
    while (buffer_count < buffer_size) {
//        unsigned char header = read_byte(file);
        unsigned char header = read_byte(str, index++);
        ...
        if (is_rle_packet) {  /* run-length packet */
            ...
            for (i = 0; i < pixel_count; i++) {
                for (j = 0; j < channels; j++) {
                    buffer[buffer_count++] = read_byte(str, index++);
                }
            }
        }
    }
...
image_t *load_image(std::stringstream &sin) {
    ...
    str.copy((char *)header, TGA_HEADER_SIZE, 0);
    ...
    if (imgtype == 2 || imgtype == 3) {           /* uncompressed */
        str.copy((char *)image->buffer, (unsigned long)calc_buffer_size(image), TGA_HEADER_SIZE);
    } else if (imgtype == 10 || imgtype == 11) {  /* run-length encoded */
        load_tga_rle(str, image);
    } else {
